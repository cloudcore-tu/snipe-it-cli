...
- 関数の責務は 1 つを基本とする。複数の関心や副作用をまたぐ場合は分割を先に検討する。
- 綺麗さ、コンパクトさ、単純さ、読みやすさ、変更容易性、機能独立性を同時に満たす設計を優先する。
...
- 設計判断、共通抽象の追加、境界変更、重複排除、test helper 整理に入る前は `design-by-contract` skill を先に読む。
- 実装前に次を明文化する。入力、出力、前提条件、不変条件、失敗条件、許可する副作用。
- 変更が複数層にまたがる場合は、どの境界で何を保証するかを先に決めてから code を触る。
- code review や refactor でも同じ。まず契約の欠落と責務混在を探し、その後に実装を直す。

## 現在の共通抽象

- `cmd/internal/run/run.go`
  - BaseOptions（client, printFlags, out は unexported。Complete() で初期化）
  - Complete → Validate → Run 制御フロー（CompleteValidateRun）
  - config 読み込みと flag override
- `cmd/internal/run/resource.go`
  - 汎用 CRUD command 生成（ResourceDef.BuildCmd）
  - sub-resource / arbitrary-path コマンドビルダー（BuildSubReadCmd, BuildPathReadCmd）
- `cmd/internal/run/validate.go`
  - 共通 validation helper（RequirePositiveInt, RequireValidJSON, ParseFilters 等）
- `cmd/internal/run/json.go`
  - JSON helper（ParseJSONObject, CheckJSONSyntax, ParseJSONBytes, EncodeJSON）
- `cmd/internal/run/httpdispatch.go`
  - HTTP verb × 出力の dispatch（FetchAndPrint, PostAndPrint, PatchAndPrint 等）
  - path injection 防止（JoinPathSegments）
- `cmd/internal/run/filedownload.go`
  - バイナリレスポンスのファイル保存（DownloadAndSave）
- `internal/snipeit`
  - HTTP client（sendAPIRequest, extractPayload）と APIError
- `internal/output`
  - output format の解釈と rendering（Printer, PrintFlags）
