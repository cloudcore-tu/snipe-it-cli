...
| コマンド | 説明 |
| --- | --- |
| `snip config init [--name NAME] --url URL --token TOKEN` | 初期設定ファイルを生成 |
| `snip config add NAME --url URL --token TOKEN` | インスタンスを追加・更新 |
| `snip config list` | 登録済みインスタンスを一覧表示（* がアクティブ） |
...
**`snip config use` コマンド（kubectl 方式）**: 設定ファイルをコマンドで書き換える方式は複数ターミナルセッションで危険。AWS CLI 方式（`SNIPE_PROFILE` 環境変数）の方が状態を持たない設計として優れるため却下。
