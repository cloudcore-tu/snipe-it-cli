...
Snipe-IT（IT資産管理 OSS）を操作する Go 製 CLI ツール。コマンド名は `snip`。
...
| バージョン表示 | `snip version` で CLI バージョン + 対応 Snipe-IT バージョンを表示 |
...
例:
snip assets list --status "Ready to Deploy"
...
```bash
snip config init [--name NAME] --url URL --token TOKEN  # 初期設定ファイル生成
snip config add NAME --url URL --token TOKEN            # インスタンス追加・更新
snip config list                                        # 一覧表示（* がアクティブ）
export SNIPE_PROFILE=NAME                                # セッション単位で切り替え
snip --profile NAME ...                                 # コマンド単位で切り替え
```
...
cmd/
  ...
  version.go            # snip version（CLI バージョン + 対応 Snipe-IT バージョン）
  config/               # snip config サブコマンド群
    config.go           # config コマンドのエントリポイント
    init.go             # snip config init
    add.go              # snip config add NAME
