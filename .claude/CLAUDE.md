...
Snipe-IT（IT資産管理 OSS）を操作する Go 製 CLI ツール。コマンド名は `snipeit`。
...
| バージョン表示 | `snipeit version` で CLI バージョン + 対応 Snipe-IT バージョンを表示 |
...
例:
snipeit assets list --status "Ready to Deploy"
snipeit assets get --id 123
snipeit assets create --data '{"name":"Laptop-001","asset_tag":"ASSET-001","model_id":1,"status_id":2}'
snipeit assets update --id 123 --data '{"status_id":3}'
snipeit assets delete --id 123 --yes
snipeit assets checkout --id 123 --data '{"checkout_to_type":"user","assigned_user":1}'
snipeit assets checkin --id 123
snipeit users list
snipeit licenses list
```
...
```bash
snipeit config init [--name NAME] --url URL --token TOKEN  # 初期設定ファイル生成
snipeit config add NAME --url URL --token TOKEN            # インスタンス追加・更新
snipeit config list                                        # 一覧表示（* がアクティブ）
export SNIPE_PROFILE=NAME                                # セッション単位で切り替え
snipeit --profile NAME ...                                 # コマンド単位で切り替え
```
...
cmd/
  ...
  version.go            # snipeit version（CLI バージョン + 対応 Snipe-IT バージョン）
  config/               # snipeit config サブコマンド群
    config.go           # config コマンドのエントリポイント
    init.go             # snipeit config init
    add.go              # snipeit config add NAME
    list.go             # snipeit config list
  internal/run/
    run.go              # BaseOptions / ParseFilters / FormatAPIError 等の共通ユーティリティ
    resource.go         # ResourceDef / ActionDef（汎用 CRUD フレームワーク）
  assets/               # snipeit assets（/api/v1/hardware）
    assets.go           # ResourceDef + checkout/checkin 追加コマンド
  users/                # snipeit users
    users.go
  licenses/             # snipeit licenses
    licenses.go
