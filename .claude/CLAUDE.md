...
Snipe-IT（IT資産管理 OSS）を操作する Go 製 CLI ツール。コマンド名は `snipe`。
...
| バージョン表示 | `snipe version` で CLI バージョン + 対応 Snipe-IT バージョンを表示 |
...
```
snipe [global flags] {resource} {verb} [flags]

例:
snipe assets list --status "Ready to Deploy"
snipe assets get --id 123
snipe assets create --data '{"name":"Laptop-001","asset_tag":"ASSET-001","model_id":1,"status_id":2}'
snipe assets update --id 123 --data '{"status_id":3}'
snipe assets delete --id 123 --yes
snipe assets checkout --id 123 --data '{"checkout_to_type":"user","assigned_user":1}'
snipe assets checkin --id 123
snipe users list
snipe licenses list
```
...
```bash
snipe config init [--name NAME] --url URL --token TOKEN  # 初期設定ファイル生成
snipe config add NAME --url URL --token TOKEN            # インスタンス追加・更新
snipe config list                                        # 一覧表示（* がアクティブ）
export SNIPE_PROFILE=NAME                                # セッション単位で切り替え
snipe --profile NAME ...                                 # コマンド単位で切り替え
```
...
cmd/
  ...
  version.go            # snipe version（CLI バージョン + 対応 Snipe-IT バージョン）
  config/               # snipe config サブコマンド群
    config.go           # config コマンドのエントリポイント
    init.go             # snipe config init
    add.go              # snipe config add NAME
    list.go             # snipe config list
  internal/run/
    run.go              # BaseOptions / ParseFilters / FormatAPIError 等の共通ユーティリティ
    resource.go         # ResourceDef / ActionDef（汎用 CRUD フレームワーク）
  assets/               # snipe assets（/api/v1/hardware）
    assets.go           # ResourceDef + checkout/checkin 追加コマンド
  users/                # snipe users
    users.go
  licenses/             # snipe licenses
    licenses.go
